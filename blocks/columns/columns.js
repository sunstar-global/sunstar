...
function applyVerticalCellAlignment(block) {
  block
    .querySelectorAll(":scope > div > div:not(.text-col-wrapper")
    .forEach((d) => {
      // this is an image column
      d.style.display = "flex";
      d.style.flexDirection = "column";
...
export default function decorate(block) {
  ...
  const videoAnchor = [...block.querySelectorAll("a")].filter((a) =>
    a.href.includes(".mp4")
  );
  ...
  const textOnlyColBlock =
    !block.querySelector("picture") && !videoAnchor.length;
